...
Font* font_create(IDWriteFactory3* dwrite_factory, const wchar_t* font_name, const float font_size, const float dpi)
{
    ...
    font->dwrite_factory = dwrite_factory;
    ...
    IDWriteFont* dw_font = NULL;
    {
        IDWriteFontFamily* font_family = NULL;
        {
            ...
            IDWriteFactory3_GetSystemFontCollection(font->dwrite_factory, &font_collection, FALSE);
    ...
    IDWriteFont_CreateFontFace(dw_font, &font->font_face);
    IDWriteFont_Release(dw_font);

    IDWriteFontFace_QueryInterface(font->font_face, &IID_IDWriteFontFace3, (void**)&font->font_face3);
...
void font_destroy(Font* font)
{
    if (font->font_face3)
    {
        IDWriteFontFace3_Release(font->font_face3);
        font->font_face3 = NULL;
    }
    if (font->font_face)
    {
        IDWriteFontFace_Release(font->font_face);
        font->font_face = NULL;
    }
    font->font_size = 0.0f;
    font->dpi = 0.0f;
...
// bound_left/top.
static uint8_t* glyph_get(const Font* font, const uint16_t codepoint, Glyph* glyph)
{
    glyph->font = font;

    // https://learn.microsoft.com/en-us/windows/win32/learnwin32/dpi-and-device-independent-pixels
    float font_em_size = font->font_size / (font->dpi / 96.0f);

    // Get glyph indices
    glyph->codepoint = codepoint;
    uint32_t codepoints[1] = { glyph->codepoint };
    UINT16 glyph_indices[1] = { 0 };
    IDWriteFontFace_GetGlyphIndices(font->font_face, codepoints, 1, glyph_indices);
    ...
    IDWriteFontFace3_GetDesignGlyphMetrics(font->font_face3, glyph_indices, 1, design_metrics, FALSE);
...
void glyph_cache_pack_codepoints(GlyphCache* glyph_cache, const Font* font, const uint32_t* codepoints, const uint16_t codepoints_length)
{
    ...
    for (int i = 0; i < codepoints_length; i++)
    {
        ...
        uint8_t* glyph_bitmap = glyph_get(font, codepoints[i], glyph);
...
void glyph_cache_init_and_fill(const HWND window, GlyphCache* glyph_cache, const wchar_t* font_family)
{
    ...
    glyph_cache_pack_codepoints(glyph_cache, font, codepoints, GLYPHS_LENGTH - 1);
