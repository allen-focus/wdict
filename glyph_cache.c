...
// bound_left/top.
static u8* glyph_get(IDWriteFactory3* dwrite_factory, Font* font, const u32 codepoint, Glyph* glyph, const u32 dpi)
{
    glyph->font = font;
...
void glyph_cache_pack_codepoints(IDWriteFactory3* dwrite_factory, GlyphCache* glyph_cache, Font* font, const u32* codepoints, const i32 codepoints_length, const u32 dpi)
{
    ...
    for (i32 i = 0; i < codepoints_length; i++)
    {
        ...
        u8* glyph_bitmap = glyph_get(dwrite_factory, font, codepoints[i], glyph, dpi);
...
void glyph_cache_init_and_fill(const HWND window, GlyphCache* glyph_cache, const wchar_t* font_family, const u32 dpi)
{
    ...
    Font* font = font_create(dwrite_factory, font_family, FONT_SIZE);
    ...
    glyph_cache_pack_codepoints(dwrite_factory, glyph_cache, font, codepoints, GLYPHS_LENGTH - 1, dpi);
    ...
    // Clean
    // font_destroy(font); // TODO: font need to be destroy but not now because glyph->font is useful
