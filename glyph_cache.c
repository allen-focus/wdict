...
// bound_left/top.
static u8* glyph_rasterize(Arena* arena, IDWriteFactory3* dwrite_factory, Font* font, const u32 codepoint, Glyph* glyph, const u32 dpi, f32 font_size)
{
...
void glyph_cache_init_and_fill(IDWriteFactory3* dwrite_factory, Font* font, GlyphCache* glyph_cache, const wchar_t* font_family, const f32 font_size, const u32 dpi)
{
    ...
    for (isize i = 0; i < GLYPHS_LENGTH - 1; i++)
    {
        GlyphAtlas* atlas = &glyph_cache->atlas;
        Glyph* glyph = &glyph_cache->glyphs[i];
        u8* glyph_bitmap = glyph_rasterize(&glyph_cache->arena, dwrite_factory, font, codepoints[i], glyph, dpi, font_size);
