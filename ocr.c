...
OcrHandler_QueryInterface(
    __FIAsyncOperationCompletedHandler_1_Windows__CMedia__COcr__COcrResult* This,
    REFIID riid, void** ppv)
{
    ...
    if (IsEqualGUID(riid, &IID_IUnknown) ||
        IsEqualGUID(riid,
            &IID___FIAsyncOperationCompletedHandler_1_Windows__CMedia__COcr__COcrResult))
    {
        *ppv = This;
    ...
    AsyncStatus asyncStatus)
{
    ...
    if (asyncStatus == Completed)
    {
        ...
        HRESULT hrGet =
            __FIAsyncOperation_1_Windows__CMedia__COcr__COcrResult_GetResults(
                asyncInfo, &result);
        ...
        if (SUCCEEDED(hrGet) && result)
        {
            ...
            {
                UINT32 lineCount = 0;
                __FIVectorView_1_Windows__CMedia__COcr__COcrLine_get_Size(
                    lines, &lineCount);

                for (UINT32 li = 0; li < lineCount && !hsText; li++)
                {
                    __x_ABI_CWindows_CMedia_COcr_CIOcrLine* line = NULL;
                    __FIVectorView_1_Windows__CMedia__COcr__COcrLine_GetAt(
                        lines, li, &line);
                    if (!line)
                        continue;
                    ...
                    {
                        UINT32 wordCount = 0;
                        __FIVectorView_1_Windows__CMedia__COcr__COcrWord_get_Size(
                            words, &wordCount);

                        for (UINT32 wi = 0; wi < wordCount && !hsText; wi++)
                        {
                            __x_ABI_CWindows_CMedia_COcr_CIOcrWord* word = NULL;
                            __FIVectorView_1_Windows__CMedia__COcr__COcrWord_GetAt(
                                words, wi, &word);
                            if (!word)
                                continue;

                            struct __x_ABI_CWindows_CFoundation_CRect bbox = { 0 };
                            __x_ABI_CWindows_CMedia_COcr_CIOcrWord_get_BoundingRect(
                                word, &bbox);
                            ...
                            if (h->cursor_x >= wx && h->cursor_x < wx + ww &&
                                h->cursor_y >= wy && h->cursor_y < wy + wh)
                            {
                                __x_ABI_CWindows_CMedia_COcr_CIOcrWord_get_Text(
                                    word, &hsText);
                                pBbox = (OcrWordBbox*)malloc(
                                    sizeof(OcrWordBbox));
                                if (pBbox)
                                {
                                    pBbox->screen_x = wx;
                        ...
                        __FIVectorView_1_Windows__CMedia__COcr__COcrWord_Release(
                            words);
                    }
            ...
            if (!hsText)
                __x_ABI_CWindows_CMedia_COcr_CIOcrResult_get_Text(
                    result, &hsText);
            ...
            if (hsText)
            {
                UINT32  wlen;
                LPCWSTR raw =
                    WindowsGetStringRawBuffer(hsText, &wlen);

                if (raw && wlen > 0)
                {
                    isize utf8_len = WideCharToMultiByte(
                        CP_UTF8, 0, raw, (i32)wlen,
                        NULL, 0, NULL, NULL);

                    if (utf8_len > 0)
                    {
                        u8* utf8_buf = (u8*)malloc(
                            (usize)utf8_len + 1);
                        if (utf8_buf)
                        {
                            WideCharToMultiByte(
                                CP_UTF8, 0, raw, (i32)wlen,
                                (char*)utf8_buf, (i32)utf8_len,
                                NULL, NULL);
                            utf8_buf[utf8_len] = 0;
...
static __FIAsyncOperationCompletedHandler_1_Windows__CMedia__COcr__COcrResultVtbl
    g_ocrHandlerVtbl = {
        OcrHandler_QueryInterface,
        OcrHandler_AddRef,
        OcrHandler_Release,
...
#define STATIC_HSTRING(Str) \
    (HSTRING)&(StaticHSTRING){ 1, (DWORD)(sizeof(Str) / sizeof(*(Str)) - 1), 0, 0, Str }

static b32 ocr_capture_screen(
    i32 x, i32 y, u32 w, u32 h,
    BITMAPINFO* out_bmi, BYTE** out_pixels, HBITMAP* out_hDib,
    i32* out_cap_x, i32* out_cap_y)
{
    ...
    *out_cap_y = cap_y;
    ...
    BYTE*   pixels = NULL;
    HBITMAP hDib =
        CreateDIBSection(hdcScreen, &bmi, DIB_RGB_COLORS,
                         (void**)&pixels, NULL, 0);
    if (!hDib)
    {
    ...
    BitBlt(hdcMem, 0, 0, (i32)cap_w, (i32)cap_h,
           hdcScreen, cap_x, cap_y, SRCCOPY);
    GdiFlush();
...
void ocr_recognize_region_async(
    i32 x, i32 y, u32 w, u32 h,
    i32 cursor_x, i32 cursor_y, HWND notify_window)
{
    BITMAPINFO bmi;
    BYTE*      pixels     = NULL;
    HBITMAP    hDib       = NULL;
    ...
    if (!ocr_capture_screen(x, y, w, h, &bmi, &pixels, &hDib,
                            &capture_x, &capture_y))
        return;
    ...
    HSTRING hsBuffer =
        STATIC_HSTRING(RuntimeClass_Windows_Storage_Streams_Buffer);
    __x_ABI_CWindows_CStorage_CStreams_CIBufferFactory* bufFactory = NULL;
    HRESULT hr = RoGetActivationFactory(
        hsBuffer, &IID___x_ABI_CWindows_CStorage_CStreams_CIBufferFactory,
        (void**)&bufFactory);
    if (FAILED(hr))
        goto cleanup_gdi;

    hr = __x_ABI_CWindows_CStorage_CStreams_CIBufferFactory_Create(
        bufFactory, bufSize, &buffer);
    if (FAILED(hr) || !buffer)
        goto cleanup;

    hr = __x_ABI_CWindows_CStorage_CStreams_CIBuffer_QueryInterface(
        buffer, &IID_IBufferByteAccess, (void**)&bufAccess);
    ...
    __x_ABI_CWindows_CStorage_CStreams_CIBuffer_put_Length(
        buffer, bufSize);
    ...
    HSTRING hsSwBmp =
        STATIC_HSTRING(RuntimeClass_Windows_Graphics_Imaging_SoftwareBitmap);
    __x_ABI_CWindows_CGraphics_CImaging_CISoftwareBitmapStatics* swBmpStatics = NULL;
    hr = RoGetActivationFactory(
        hsSwBmp,
        &IID___x_ABI_CWindows_CGraphics_CImaging_CISoftwareBitmapStatics,
        (void**)&swBmpStatics);
    if (FAILED(hr))
        goto cleanup;

    hr = __x_ABI_CWindows_CGraphics_CImaging_CISoftwareBitmapStatics_CreateCopyWithAlphaFromBuffer(
        swBmpStatics, buffer, BitmapPixelFormat_Bgra8, (i32)capW, (i32)capH,
        BitmapAlphaMode_Ignore, &swBitmap);
    ...
    hr = RoGetActivationFactory(
        hsOcr, &IID___x_ABI_CWindows_CMedia_COcr_CIOcrEngineStatics,
        (void**)&ocrStatics);
    if (FAILED(hr))
        goto cleanup;

    hr = __x_ABI_CWindows_CMedia_COcr_CIOcrEngineStatics_TryCreateFromUserProfileLanguages(
        ocrStatics, &engine);
    if (FAILED(hr) || !engine)
    {
        Assert(0);
    ...
    hr = __x_ABI_CWindows_CMedia_COcr_CIOcrEngine_RecognizeAsync(
        engine, swBitmap, &asyncOp);
    if (FAILED(hr) || !asyncOp)
        goto cleanup;

    {
        OcrCompletedHandler* handler =
            (OcrCompletedHandler*)calloc(1, sizeof(OcrCompletedHandler));
        if (!handler)
            goto cleanup;
    ...
    if (swBmpStatics)
    {
        __x_ABI_CWindows_CGraphics_CImaging_CISoftwareBitmapStatics_Release(
            swBmpStatics);
        swBmpStatics = NULL;
    }
